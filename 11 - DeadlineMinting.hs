...
module Batch50deadlineMinting where
...
mint mp = do
    let ptime = POSIXTime (1596059111000 :: Integer)
    now <- currentTime
...
test = runEmulatorTraceIO $ do
    ...
    callEndpoint @"mint" h1 $ MintParams
                    { mpTokenName = "batch50coin"
                    , mpAmount    = 1000
                    }
    void $ Emulator.waitNSlots 10
    callEndpoint @"mint" h2 $ MintParams
                    { mpTokenName = "batch50coin2"
                    , mpAmount    = 2000
                    }
    void $ Emulator.waitNSlots 11
    callEndpoint @"mint" h1 $ MintParams
                    { mpTokenName = "batch50coin3"
                    , mpAmount    = 3000
                    }
    void $ Emulator.waitNSlots 10
    callEndpoint @"mint" h2 $ MintParams
                    { mpTokenName = "batch50coin4"
                    , mpAmount    = 4000
                    }
    void $ Emulator.waitNSlots 11
