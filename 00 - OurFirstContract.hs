...
import qualified Ledger.Scripts      as Scripts               -- Low Level Typed Validator (on-chain code)
...
{-# INLINABLE alwaysSucceeds #-} -- Everything that its supposed to run in on-chain code need this pragma
            ...
            --      👇 imported from Ledger.Scripts   
alwaysSucceeds :: BuiltinData -> BuiltinData -> BuiltinData -> ()   -- the value of this function is on its side effects, so we ignore the result with ()
            --    👆 Datum         👆 Redeemer    👆 ScriptCtxt 
...
-- 👆 👇 Used in tandem. 
-- validator needs to compile  👇 alwaysSucceeds/alwaysFails into Plutus Core 
-- Then mkValidatorScript presents it to the off-chain code
validator :: Validator
--          👇 mkValidatorScript :: CompiledCode (BuiltinData -> BuiltinData -> BuiltinData -> ()) -> Validator
validator = mkValidatorScript $$(PlutusTx.compile [|| alwaysSucceeds ||])  --2nd example change this to alwaysFails
-- compiles inlinable fn to Plutus Core 👆         👆 Oxford brackets for template haskell for inlinable function 

valHash :: Ledger.ValidatorHash -- The ref to the Script is done through the ValHash
valHash = Scripts.validatorHash validator  -- Scripts also comes from Ledger.Scripts, these are low-level types (there is another fn w/same name that is high-level type). The hash of the validator
...
-- THE OFFCHAIN CODE
-- You program the endpoints in your contract and present it through a Schema. 

type GiftSchema =
            Endpoint "give" Integer  --An Integer Parameter
        .\/ Endpoint "grab" () 

give :: AsContractError e => Integer -> Contract w s e () -- the give endpoint is a fn that creates a contract
give amount = do
    -- Using the contract monad, this fn will create a contract 👇
    ...
    -- Then it will 👇 submit the contract
...
--                                       👇  if there was a redeemer you would include a Integer -> for example (it doesn't because it alwaysSucceeds or fails)
